...
const emit = defineEmits<{
    (e: "onPageCount", count: number): void;
}>();

const song = store.song(props.song.id)!;
...
const insertChord = (sectionIndex: number, chordIndex: number) => {
    ...
    song.sections[sectionIndex].progression.splice(chordIndex + 1, 0, newChord);
};
...
const duplicateProgression = (index: number) => {
    const element = song.sections[index];
    ...
    song.sections.splice(index + 1, 0, newElement);
};
...
const updatePages = () => {
    ...
    const bottomMinusPadding =
        sectionParent.clientHeight -
        Number.parseFloat(computedParent.paddingBottom);
...
<template>
    ...
    >
        ...
        >
            ...
            <template v-else>
                <div
                    v-for="(element, index) in song.sections.slice(
                        section.start,
                        section.end
                    )"
                    :key="index"
